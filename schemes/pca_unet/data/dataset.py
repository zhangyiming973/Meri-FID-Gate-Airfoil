"""PCA-UNet 数据集：从 NPZ 加载 SDF、语义掩码与条件向量。

``MeridianSDFDataset`` 为 AE 训练提供 (x, sdf, semantic, condition) 批次；
条件向量经 ``ConditionStats`` 标准化后供扩散模型使用。
"""
...
def resolve_npz_path(row: pd.Series, npz_dir: Path) -> Path:
    ...
    优先使用 ``npz_dir/<sample_id>.npz``，否则回退到索引表中的 ``npz_file`` / ``npz_path`` 字段。
    """
    local = npz_dir / f"{row['sample_id']}.npz"
    if local.exists():
        return local
    npz_file = str(row.get("npz_file", ""))
    if npz_file:
        rel = npz_dir / npz_file
        if rel.exists():
            return rel
        direct = Path(npz_file)
        if direct.exists():
            return direct
...
def load_condition_vector(
    ...
    Args:
        sample_id: 样本 ID。
        index_row: 数据划分索引中的一行。
        param_table: 可选的全局参数表。
        npz_path: NPZ 路径，用于读取内嵌条件 JSON。
        condition_columns: 本数据集使用的条件列；默认使用 Meridian 条件列。
...
class MeridianSDFDataset(Dataset):
    """子午面 SDF 数据集，供自编码器训练使用。

    每个样本返回：
      - ``x``: 输入通道（SDF ± 语义掩码）
      - ``sdf`` / ``semantic``: 重建目标与语义监督
      - ``condition`` / ``condition_raw``: 标准化与原始条件
      - ``physics``: 最小壁厚、面积等物理摘要（用于损失）
    """
    ...
    def __init__(
        ...
        self.condition_columns = condition_stats.columns if condition_stats else CONDITION_COLUMNS
    ...
    def __getitem__(self, idx: int) -> dict[str, Any]:
        ...
        semantic_raw = data["semantic_mask"].astype(np.float32)
        # Meridian 语义为 0-5 区域标签；翼型第一版常用二值内外 mask。
        semantic = semantic_raw / 5.0 if semantic_raw.max(initial=0.0) > 1.0 else semantic_raw
        physics = json.loads(str(data["physics_summary_json"])) if "physics_summary_json" in data else {}

        cond_raw = load_condition_vector(sample_id, row, self.param_table, npz_path, self.condition_columns)
        cond_arr = np.array([cond_raw[c] for c in self.condition_columns], dtype=np.float32)
        cond_norm = (
