"""dim_unet 第二阶段：在 PCA 网格上以 ConditionVector 为条件训练 UNet 扩散模型。"""
...
from schemes.dim_unet.data.dataset import ConditionStats
from schemes.dim_unet.losses.ae_losses import physics_risk_proxy
from schemes.dim_unet.models.autoencoder import MeridianAutoEncoder
from schemes.dim_unet.models.diffusion import DiffusionSchedule
from schemes.dim_unet.models.latent_pca import LatentPCA, save_latent_pca
from schemes.dim_unet.models.pca_unet import build_pca_unet
from schemes.dim_unet.records.body_latent import load_records
from schemes.dim_unet.train.diffusion_codec import DiffusionLatentCodec
from schemes.dim_unet.utils.paths import project_root
from schemes.dim_unet.utils.visualization import (
...
def _eval_generation_l1(
    ...
    for r in test_recs[:max_samples]:
        ...
        z_gen_enc = schedule.sample(
            ...
            steps=unet_cfg.get("sample_steps", 80),
...
def train_diffusion(cfg: dict[str, Any], ae_run_dir: Path, run_dir: Path) -> Path:
    """在 AE 潜空间 PCA 网格上训练尺寸条件 UNet 扩散模型。"""
    root = project_root()
    ...
    pca_dim = unet_cfg.get("pca_dim", 192)
    pca = LatentPCA.fit(z_train_np, pca_dim)
    save_latent_pca(pca, run_dir / "latent_pca.json")
    print(f"[dim_unet] PCA: {z_shape[1:]} -> {pca.dim} dims, std={pca.std:.4f}")

    codec = DiffusionLatentCodec.from_mode("dim_unet", pca, z_shape)
    print(f"[dim_unet] grid: {codec.grid.channels}×{codec.grid.height}×{codec.grid.width}")
    ...
    best_gen = float("inf")
    sample_shape = codec.sample_shape(1)
    grid_meta = {"channels": grid_spec.channels, "height": grid_spec.height, "width": grid_spec.width}
    ...
    for epoch in range(1, epochs + 1):
        unet.train()
        ep_loss, nb = 0.0, 0
        for z0, cond in tqdm(train_loader, desc=f"dim_unet {epoch}/{epochs}", leave=False):
        ...
        if epoch == 1 or epoch % 20 == 0 or epoch == epochs:
            print(f"[dim_unet] ep{epoch}: train={history['train_loss'][-1]:.5f} val={val_loss:.5f} gen_l1={gen_l1:.4f}")

    plot_training_curves(history, run_dir / "training_curves.png", "dim_unet Diffusion Training")
    plot_training_dashboard(history, run_dir / "training_dashboard.png", "dim_unet Training Dashboard")
    ...
    with torch.no_grad():
        for r in test_recs:
            ...
            plot_sdf_panel(gt_np, gen_np, f"dim_unet Gen {r.sample_id}", vis / f"{r.sample_id}.png", cond_dict)
    ...
    plot_sample_grid(grid_items, run_dir / "grid_test.png", "dim_unet Generations vs Original SDF", max_samples=10)
    plot_per_sample_metrics(
        [{"sample_id": m["sample_id"], "l1": m["l1_vs_original"]} for m in metrics],
        run_dir / "metrics_test.png",
        "Per-sample dim_unet Generation L1",
    )
    ...
    save_json(
        {
            ...
            "pca_dim": pca.dim,
            "denoiser": "dim_unet",
            "condition_dim": len(cond_stats.columns),
            "grid": grid_meta,
        },
        run_dir / "summary.json",
    )
    print(f"[dim_unet] done -> {run_dir} | mean_gen_l1={mean_l1:.4f} (AE recon={mean_recon:.4f})")
    return run_dir
