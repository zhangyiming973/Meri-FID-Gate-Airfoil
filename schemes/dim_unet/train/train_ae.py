...
from schemes.dim_unet.data.dataset import ConditionStats, MeridianSDFDataset
from schemes.dim_unet.data.splits import load_condition_stats, load_splits
from schemes.dim_unet.gate.latent_gate import GateConfig, LatentRepresentationGate
from schemes.dim_unet.losses.ae_losses import AELossWeights, compute_ae_losses
from schemes.dim_unet.models.autoencoder import MeridianAutoEncoder
from schemes.dim_unet.records.body_latent import BodyLatentRecord, save_records
...
def train_autoencoder(cfg: dict[str, Any], run_dir: Path) -> Path:
    ...
    # 加载参数表（CSV / Excel）、划分与条件归一化统计
    param_path = Path(cfg.get("param_table_path", ""))
    excel_path = Path(cfg.get("condition_excel_path", ""))
    param_table_path = param_path if param_path.is_file() else (excel_path if excel_path.is_file() else None)
    param_table = None
    if param_table_path:
        suffix = param_table_path.suffix.lower()
        param_table = pd.read_excel(param_table_path) if suffix in (".xlsx", ".xls") else pd.read_csv(param_table_path)
    ...
    train_ds = MeridianSDFDataset(
        train_df, npz_dir, param_table, param_table_path, cond_stats, ae_cfg["use_semantic"]
    )
    test_ds = MeridianSDFDataset(
        test_df, npz_dir, param_table, param_table_path, cond_stats, ae_cfg["use_semantic"]
    )
    ...
    for epoch in range(1, ae_cfg["epochs"] + 1):
        ...
        for batch in tqdm(train_loader, desc=f"AE {epoch}/{ae_cfg['epochs']}", leave=False):
            ...
            opt.zero_grad()
            _, recon = model(x)
            losses = compute_ae_losses(recon, sdf, sem, batch["physics"], lw, r_grid=r_grid)
