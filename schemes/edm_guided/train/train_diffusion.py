"""EDM 条件扩散训练：在 PCA 压缩潜空间上用预条件化 MLP 学习条件生成。

采用 Karras EDM 框架：对数正态噪声分布、预条件化损失、Heun 二阶采样 + CFG。
"""
...
from schemes.edm_guided.models.autoencoder import MeridianAutoEncoder
from schemes.edm_guided.models.edm_schedule import EDMDiffusionSchedule, resolve_latent_edm_config
from schemes.edm_guided.models.latent_pca import LatentPCA, save_latent_pca
from schemes.edm_guided.models.mlp_denoiser import EDMLDenoiser
from schemes.edm_guided.records.body_latent import load_records
from schemes.edm_guided.train.diffusion_codec import DiffusionLatentCodec
from schemes.edm_guided.utils.paths import project_root
from schemes.edm_guided.utils.visualization import (
...
def _build_denoiser(diff_cfg: dict, codec: DiffusionLatentCodec, cond_dim: int, device: torch.device):
    return (
        EDMLDenoiser(codec.pca.dim, cond_dim, hidden=diff_cfg.get("mlp_hidden", 512)).to(device),
        "edm_mlp",
    )
...
def eval_generation_l1(
    ...
    for r in test_recs[:max_samples]:
        ...
        z_gen_enc = schedule.sample(
            ...
            steps=diff_cfg.get("sample_steps", 35),
        )
...
def train_diffusion(cfg: dict[str, Any], ae_run_dir: Path, run_dir: Path) -> Path:
    ...
    z_for_sigma = z_enc if isinstance(z_enc, torch.Tensor) else torch.from_numpy(z_enc)
    sigma_data = edm_cfg.get("sigma_data")
    if sigma_data is None:
        sigma_data = EDMDiffusionSchedule.estimate_sigma_data(z_for_sigma)
    edm_resolved = resolve_latent_edm_config(edm_cfg, sigma_data)
    print(
        f"EDM sigma_data={edm_resolved['sigma_data']:.4f} "
        f"sigma_max={edm_resolved['sigma_max']:.2f} (auto={edm_cfg.get('sigma_max') is None})"
    )
    ...
    schedule = EDMDiffusionSchedule(
        sigma_data=edm_resolved["sigma_data"],
        sigma_min=edm_resolved["sigma_min"],
        sigma_max=edm_resolved["sigma_max"],
        p_mean=edm_resolved["p_mean"],
        p_std=edm_resolved["p_std"],
        rho=edm_resolved["rho"],
        device=device,
    )
    ...
    def eval_noise_loss() -> float:
        ...
        with torch.no_grad():
            for r in test_recs:
                ...
                cond = torch.from_numpy(r.condition_norm).unsqueeze(0).to(device)
                losses.append(float(schedule.training_loss(denoiser, z0, cond)))
        return float(np.mean(losses))

    print(
        f"EDM training: p_mean={schedule.p_mean} p_std={schedule.p_std} "
        f"sigma=[{schedule.sigma_min}, {schedule.sigma_max}] rho={schedule.rho} "
        f"sample_steps={diff_cfg.get('sample_steps', 35)} cfg_scale={diff_cfg.get('cfg_scale', 1.5)}"
    )

    for epoch in range(1, diff_cfg["epochs"] + 1):
        denoiser.train()
        ep_loss, nb = 0.0, 0
        for z0, cond in tqdm(train_loader, desc=f"EDM {epoch}/{diff_cfg['epochs']}", leave=False):
            z0, cond = z0.to(device), cond.to(device)
            drop = torch.rand(z0.shape[0], device=device) < diff_cfg["cfg_dropout"]
            loss = schedule.training_loss(denoiser, z0, cond, cond_mask=(~drop).float())
        ...
        if epoch == 1 or epoch % 20 == 0 or epoch == diff_cfg["epochs"]:
            print(f"[EDM] ep{epoch}: train={history['train_loss'][-1]:.5f} val={val_loss:.5f} gen_l1={gen_l1:.4f}")

    plot_training_curves(history, run_dir / "training_curves.png", "EDM Diffusion Training")
    plot_training_dashboard(history, run_dir / "training_dashboard.png", "EDM Diffusion Training Dashboard")
    ...
    plot_sample_grid(grid_items, run_dir / "grid_test.png", "EDM Diffusion Generations vs Original SDF", max_samples=10)
    plot_per_sample_metrics(
    ...
    save_json(
        {
            ...
            "edm": {
                ...
                "p_mean": schedule.p_mean,
                "p_std": schedule.p_std,
                "rho": schedule.rho,
                "sample_steps": diff_cfg.get("sample_steps", 50),
                "cfg_scale": diff_cfg.get("cfg_scale", 1.2),
            },
        },
        run_dir / "summary.json",
    )
    print(f"EDM diffusion done -> {run_dir} | mean_gen_l1={mean_l1:.4f} (AE recon={mean_recon:.4f})")
    return run_dir
